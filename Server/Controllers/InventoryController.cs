...
namespace Server.Controllers
{
    ...
    public class InventoryController : ControllerBase
    {
        ...
        public InventoryController(IProductInventoryService inventoryService, IPartnerService partnerService)
        {
        ...
        public async Task<IActionResult> CreateInventory([FromBody] CreateInventoryDTO inventory)
        {
            ...
            return CreatedAtAction(nameof(GetInventory), new { id = ((InventoryDTO)response.Data).Id }, response);
        }

        // PUT: api/inventory/5
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateInventory(int id, [FromBody] UpdateInventoryDTO inventory)
        {
            var identity = HttpContext.User.Identity as ClaimsIdentity;
            var partner = await _partnerService.FindByClaim(identity);
            if (id != inventory.Id)
        ...
        public async Task<IActionResult> GetInventoriesByProductId(int productId)
        {
            ...
            var response = await _inventoryService.GetInventoriesByProductIdAsync(productId, partner);
            if (!response.Flag)
            {
                return NotFound(response);
            }
